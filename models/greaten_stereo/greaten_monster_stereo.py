...
from models.greaten_stereo.updaters import disp_warp_mono, REMP, BasicMultiUpdateBlock, MonoDepthMultiUpdateBlock
...
from models.greaten_stereo.basic_modules import context_upsample, compute_scale_shift, Conv2x, Conv2xIN, BasicConv, BasicConvIN
...
class GREATENStereo(nn.Module):
    def __init__(self, args: argparse.Namespace):
        ...
        self.update_block_mix_stereo = MonoDepthMultiUpdateBlock(self.args, channels=args.channels)
        self.update_block_mix_mono = MonoDepthMultiUpdateBlock(self.args, channels=args.channels)
    ...
    def forward(self, left_img: torch.Tensor, right_img: torch.Tensor, iters: int=12, disp_init: torch.Tensor=None, test_mode: bool=False) -> Tuple[Union[torch.Tensor, Union[List[torch.Tensor], dict, int, float]]]:
        ...
        depth_mono, feat_mono_left, feat_mono_right = self.infer_mono(left_img, right_img)
        ...
        with autocast(enabled=self.args.mixed_precision, dtype=getattr(torch, self.args.precision_dtype, torch.float16)):
            ...
            cnet_list = self.context_neck(context_left_list, stem_x_list)
        ...
        for iter in range(iters):
            ...
            # Start to iteratively update the mono depth-based disparity.
            if iter > int(iters - 8):
                # Convert the relative mono depth into disparity.
                if iter == int(iters - 7):
                    bs, _, _, _ = disp.shape
                    for i in range(bs):
                ...
                # Prepare the features for united mono depth-based disparity and original disparity updating.
                warped_right_mono = disp_warp_mono(feat_right[0], disp_mono_4x.clone().to(feat_right[0].dtype))[0]
                flaw_mono = warped_right_mono - feat_left[0]

                warped_right_stereo = disp_warp_mono(feat_right[0], disp.clone().to(feat_right[0].dtype))[0]
                flaw_stereo = warped_right_stereo - feat_left[0]
                geo_feat_mono = cv_fn(disp_mono_4x, coords)
            ...
            if iter == iters - 1:
                refine_value = self.remp(disp_mono_4x_up, disp_up, left_img, right_img)
